...
async function createEmployee(req, res) {
  ...
  if (phone) {
    ...
    if (digits.length !== 10) return err(res, "Mobile number must be 10 digits");
  ...
  if (cleanPhone) {
    const { data: phoneExisting } = await sb.from("users").select("id").eq("phone", cleanPhone).single();
    if (phoneExisting) return err(res, "An account with that mobile number already exists", 409);
  }
  ...
  const { data: newUser, error } = await sb.from("users").insert({
    name, email: email.toLowerCase().trim(), phone: cleanPhone,
  ...
  return ok(res, {
    id: newUser.id, name, email: newUser.email, phone: cleanPhone,
